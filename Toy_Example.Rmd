...
knots_seq <- seq(from = tau_seq[1] - 0.02, to = tau_seq[b] + 0.02, length.out = K) %>% .[-c(1, (K))]
boundary <- c(tau_seq[1] - 0.02, tau_seq[b] + 0.02)
Phi <- bs(tau_seq, knots = knots_seq, degree = 1, intercept = TRUE, Boundary.knots = boundary)
...
add_decomp <- function(delta, lambda_1, lambda_2, tol_error, max_iter, X, Y, V, Phi) {
  ...
  theta_old <- eta_old
  alpha_old <- matrix(0.8, nrow = p+1, ncol = m)
...
```{r}
theta_init <- matrix(0, nrow = (p+1)*K, ncol = m)

nz_idx <- which(sp_mat != 0, arr.ind = TRUE)
for(i in 1:nrow(nz_idx)) {
  j <- nz_idx[i, "row"] - 1
  g <- nz_idx[i, "col"]
  theta_init[(j*K +1):((j+1)*K), g] <- 5
}

result <- add_decomp(delta, lambda_1, lambda_2, tol_error, max_iter,  X = X, Y = Y, V = V, Phi = Phi,
                     theta_0 = theta_init, alpha_0 = LR_mat)
```
...
```{r}
check_sp_table(true = sp_mat, est = gamma_tau_hat, table = TRUE, tol = 0.1^5)
```
...
```{r}
est_sp <- gamma_tau_hat[[3]]
for(i in 1:nrow(est_sp)) {
  for(j in 1:ncol(est_sp)) {
    est_sp[i, j] <- ifelse(est_sp[i, j] != 0, est_sp[i, j] + 4, est_sp[i, j])
  }
}

beta_hat <- result$alpha + est_sp
norm(beta_hat - true_B, "F")/norm(true_B, "F")

Y_hat <- X %*% beta_hat
norm(Y - Y_hat, "F")/norm(Y, "F")
```
...
error <- list()
for(l in 1:b) {
  error[[l]] <- Y - X%*%lasso_coef
}

cal_cl_sum(error, tau_seq)
```
