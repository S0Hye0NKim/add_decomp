...
for(k in 1:10) {
  ...
  K <- 5
  b <- 15
  tau_seq <- seq(from = 0.65, to = 0.95, length.out = b)
  idx_tau <- (tau_seq >= "0.7" & tau_seq <= "0.9")
  ...
  QR_Lasso <- foreach(g = 1:m) %dopar% {
    ...
    QR_model <- rqPen::rq.lasso.fit.mult(x = train_X[, -1], y = train_Y[, g], tau_seq = tau_seq, lambda = 0.0001)
...
save.image(file = "ksh_CCLE_PE_upper.RData")
