...
$$\sum_\ell^{b}\sum_{g=1}^m\sum_{i=1}^n\rho_\tau(Y_i^{(g)}-x_i^T[\alpha^{(g)}+\gamma^{(g)}(\tau)])\\=\sum_\ell^{b}\sum_{g=1}^m\sum_{i=1}^n\rho_\tau(e_i^{(\ell)(g)})$$
...
```{r}
names(result$e) <- tau_seq

cl_sum <- list()
for(i in 1:length(result$e)) {
  cl_sum[[i]] <- check_ft(result$e[[i]], tau = tau_seq[i])
}

lapply(cl_sum, FUN = function(x) data.frame(cl_sum = apply(x, MARGIN = 2, FUN = sum) %>% sum)) %>%
  `names<-`(value = tau_seq) %>%
  bind_rows(.id = "tau") %>%
  summarise(cl_sum = sum(cl_sum))
```
...
cl_sum <- list()
for(i in 1:length(est_e)) {
  cl_sum[[i]] <- check_ft(est_e[[i]], tau = tau_seq[i])
}
