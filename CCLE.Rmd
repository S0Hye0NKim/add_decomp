...
sum(gamma_sing_val[1:1])/sum(gamma_sing_val)
...
```{r}
lamb_1_seq <- seq(from = 0.005, to = 0.1, by = 0.005)
simul_lamb1 <- list()

for(idx in 1:length(lamb_1_seq)) {
  simul_lamb1[[idx]] <- add_decomp(delta = 0.005, lambda_1 = lamb_1_seq[idx], lambda_2 = 0.2,
...
lapply(alpha_hat, FUN = function(x) svd(x) %>% .$d) %>%
  lapply(FUN = function(x) data.frame(ratio = (sum(x[1:3])/sum(x)), rank = sum(abs(x) > 0.1^5))) %>%
...
```{r}
alpha_col %>% lapply(FUN = function(x) data.frame(GENE = colnames(X), alpha_hat = x) %>% arrange(-alpha_hat)) %>% `names<-`(value = lamb_1_seq) %>% lapply(FUN = function(x) x[9, ]) %>% bind_rows(.id = "lambda_1")
```
