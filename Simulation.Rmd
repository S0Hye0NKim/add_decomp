...
for(i in 1:(cores[1]-2)) {
  ...
  lin_model <- lm(Y_modified~., data = data.frame(X[, -1]))
  alpha_init <- lin_model$coefficients
  result <- add_decomp(delta = 1, lambda_1 = lamb1_seq[i], lambda_2 = 20, tol_error = 0.1, max_iter = 300,
                       X = X, Y = Y, V = V, Phi = Phi, 
                     theta_0 = theta_init, alpha_0 = alpha_init, tau_seq = tau_seq)
  simulation[[i]] <- result
}
...
cl <- makeCluster(cores[1]-2) #not to overload your computer
registerDoParallel(cl) # Ready to parallel
lamb2_seq <- c(seq(5, 60, by = 5), seq(70, 200, by = 20))
...
simulation <- foreach(lambda_2 = lamb2_seq, .noexport = "add_decomp") %dopar% {
  ...
  result <- add_decomp(delta = 1, lambda_1 = 4, lambda_2 = lambda_2, tol_error = 0.1,max_iter = 300,
                       X = X, Y = Y, V = V, Phi = Phi, 
                     theta_0 = theta_init, alpha_0 = alpha_init, tau_seq = tau_seq)
  result
}
