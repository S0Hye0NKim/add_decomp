...
for(simul in 1:simul_times) {
  ...
  log_lamb1 <- c( seq(-3, -0.9, length.out = 20))
  lamb1_seq <- exp(log_lamb1)
  log_lamb2 <- c(seq(4, 5.5, length.out = 20))
...
save(simul_eq_add_decomp_1, simul_eq_LR_model_1, simul_eq_SP_model_1, 
     LR_mat, sp_mat, Phi, tau_seq, tau_seq_real, X_list, file = "ksh_simul_mixture_error_equal_1.RData")
