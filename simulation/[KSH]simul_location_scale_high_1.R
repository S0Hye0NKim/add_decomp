...
simul_high_add_decomp_1 <- vector("list", length = simul_times)
for(simul in 1:simul_times) {
  ...
  simul_high_add_decomp_1[[simul]] <- result
}
...
registerDoParallel(cl) # Ready to parallel
simul_high_LR_model_1 <- foreach(simul = 1:simul_times, .noexport = "add_decomp") %dopar% {
  library(dplyr)
  library(splines)
  library(Matrix)
  library(glmnet)
...
simul_high_SP_model_1 <- vector("list", length = simul_times)
for(simul in 1:simul_times) {
  ...
  simul_high_SP_model_1[[simul]] <- result
}
...
save(simul_high_add_decomp_1, simul_high_LR_model_1, simul_high_SP_model_1, 
     LR_mat, sp_mat, Phi, tau_seq, tau_seq_real, X_list, file = "ksh_simul_location_scale_high_1.RData")
