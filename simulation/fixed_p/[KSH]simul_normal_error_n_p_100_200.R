...
for(simul in 1:simul_times) {
  ...
  log_lamb1 <- c( seq(1.5, 2, length.out = 20))
  lamb1_seq <- exp(log_lamb1)
  log_lamb2 <- c(seq(4.2, 4.6, length.out = 20))
...
for(simul in 1:simul_times) {
  ...
  log_lamb <- c(seq(3, 4.6, length.out = 20))
...
save(simul_add_decomp, simul_LR_model, simul_SP_model, est_gamma, check_sp_table, 
     LR_mat, sp_mat, Phi, tau_seq, tau_seq_real, X_list, file = "ksh_simul_normal_error_n_p_100_200.RData")