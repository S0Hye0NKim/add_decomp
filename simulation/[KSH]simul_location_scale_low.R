...
for(simul in 1:simul_times) {
  ...
  eps_list[[simul]] <- matrix(rnorm(n*m, mean = 0, sd = 0.5), nrow = n, ncol = m)
}
...
simul_low_add_decomp <- foreach(simul = 1:simul_times, .noexport = "add_decomp") %dopar% {
  ...
  init_val <- add_decomp_r(delta = 1, lambda_1 = 0.04, lambda_2 = 0.1, tol_error = 0.1^5, max_iter = 50,
                           X = X, Y = Y, V = V, Phi = Phi, 
                           theta_0 = theta_init, Z_0 = X%*%alpha_init, tau_seq = tau_seq, weight = FALSE)
  
  lamb1_seq <- c( seq(0.01, 0.1, length.out = 10))
  lamb2_seq <- c(seq(80, 300, length.out = 10))
...
simul_low_SP_model <- foreach(simul = 1:simul_times, .noexport = "add_decomp") %dopar% {
  ...
  lamb_seq <- seq(0.1, 2, length.out = 20)
