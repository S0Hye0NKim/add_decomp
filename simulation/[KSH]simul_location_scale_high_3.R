...
simul_high_add_decomp_3 <- vector("list", length = simul_times)
for(simul in 1:simul_times) {
  ...
  log_lamb1 <- c( seq(1, 5, length.out = 20))
  lamb1_seq <- exp(log_lamb1)
  log_lamb2 <- c( seq(4, 6.2, length.out = 20))
  ...
  BIC_params <- BIC_table %>% lapply(FUN = function(x) bind_rows(x)) %>%
    ...
    filter(S_hat_net != 0) %>%
    arrange(BIC) %>% filter(lambda_2 < 200) %>% 
    head(1)
  ...
  simul_high_add_decomp_3[[simul]] <- result
}
...
registerDoParallel(cl) # Ready to parallel
simul_high_LR_model_3 <- foreach(simul = 1:simul_times, .noexport = "add_decomp") %dopar% {
  library(dplyr)
  library(splines)
  library(Matrix)
  library(glmnet)
...
simul_high_SP_model_3 <- vector("list", length = simul_times)
for(simul in 1:simul_times) {
  ...
  log_lamb <- c( seq(4, 6.2, length.out = 20))
  ...
  simul_high_SP_model_3[[simul]] <- result
}
...
save(simul_high_add_decomp_3, simul_high_LR_model_3, simul_high_SP_model_3, 
     LR_mat, sp_mat, Phi, tau_seq, tau_seq_real, X_list, file = "ksh_simul_location_scale_high_3.RData")
