...
simul_eq_add_decomp_1 <- vector("list", length = simul_times)
for(simul in 1:simul_times) {
  ...
  log_lamb1 <- c( seq(-3, -1.2, length.out = 20))
  lamb1_seq <- exp(log_lamb1)
  log_lamb2 <- c(seq(4, 5.3, length.out = 20))
  ...
  simul_eq_add_decomp_1[[simul]] <- result
}
...
registerDoParallel(cl) # Ready to parallel
simul_eq_LR_model_1 <- foreach(simul = 1:simul_times, .noexport = "add_decomp") %dopar% {
  library(dplyr)
  library(splines)
  library(Matrix)
  library(glmnet)
...
simul_eq_SP_model_1 <- vector("list", length = simul_times)
for(simul in 1:simul_times) {
  ...
  log_lamb <- c(seq(-3, 5, length.out = 20))
  ...
  simul_eq_SP_model_1[[simul]] <- result
}
...
save(simul_eq_add_decomp_1, simul_eq_LR_model_1, simul_eq_SP_model_1, 
     LR_mat, sp_mat, Phi, tau_seq, tau_seq_real, X_list, file = "ksh_simul_laplace_error_equal_1.RData")
