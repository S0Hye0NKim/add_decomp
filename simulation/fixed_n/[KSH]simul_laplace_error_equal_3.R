...
simul_eq_add_decomp_3 <- vector("list", length = simul_times)
for(simul in 1:simul_times) {
  ...
  simul_eq_add_decomp_3[[simul]] <- result
}
...
registerDoParallel(cl) # Ready to parallel
simul_eq_LR_model_3 <- foreach(simul = 1:simul_times, .noexport = "add_decomp") %dopar% {
  library(dplyr)
  library(splines)
  library(Matrix)
  library(glmnet)
...
simul_eq_SP_model_3 <- vector("list", length = simul_times)
for(simul in 1:simul_times) {
  ...
  simul_eq_SP_model_3[[simul]] <- result
}
...
save(simul_eq_add_decomp_3, simul_eq_LR_model_3, simul_eq_SP_model_3, 
     LR_mat, sp_mat, Phi, tau_seq, tau_seq_real, X_list, file = "ksh_simul_laplace_error_equal_3.RData")
