...
for(simul in 1:simul_times) {
  ...
  log_lamb1 <- c( seq(6, 6.4, length.out = 20))
  lamb1_seq <- exp(log_lamb1)
  log_lamb2 <- c(seq(5.3, 5.8, length.out = 20))
...
save(simul_add_decomp, simul_LR_model, simul_SP_model, est_gamma, 
     LR_mat, sp_mat, Phi, tau_seq, tau_seq_real, X_list, file = "ksh_simul_location_scale_n_p_400_400.RData")
