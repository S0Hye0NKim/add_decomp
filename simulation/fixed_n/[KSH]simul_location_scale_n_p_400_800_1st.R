...
for(simul in 1:simul_times) {
  ...
  log_lamb1 <- c( seq(3, 4.6, length.out = 20))
  lamb1_seq <- exp(log_lamb1)
  log_lamb2 <- c(seq(4.6, 5.1, length.out = 20))
...
for(simul in 1:simul_times) {
  ...
  log_lamb <- c(seq(3, 5.1, length.out = 20))
...
save(simul_add_decomp, simul_LR_model, simul_SP_model, est_gamma, check_sp_table, 
     LR_mat, sp_mat, Phi, tau_seq, tau_seq_real, X_list, file = "ksh_simul_location_scale_n_p_400_800_1st.RData")