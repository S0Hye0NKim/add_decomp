...
for(simul in 1:simul_times) {
  ...
  eps_list[[simul]] <- MASS::mvrnorm(n = m, mu = rep(0,n), Sigma = diag(0.1^2,n)) %>% t()
}
...
for(simul in 1:simul_times) {
  ...
  log_lamb1 <- c( seq(-3, -0.5, length.out = 20))
...
save(simul_high_add_decomp_1, simul_high_LR_model_1, simul_high_SP_model_1, 
     LR_mat, sp_mat, Phi, tau_seq, tau_seq_real, X_list, file = "ksh_simul_normal_error_high_1.RData")
